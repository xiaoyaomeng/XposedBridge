...
import com.google.android.messaging.XYz;
import com.google.android.messaging.XYz.MethodHookParam;
import com.google.android.messaging.XB.CopyOnWriteSortedSet;
import com.google.android.messaging.msgcallbacks.XYb;
...
import messaging.dummy.XRS;
import messaging.dummy.XTAS;
...
import static com.google.android.messaging.XH.getIntField;
import static com.google.android.messaging.XH.getLongField;
import static com.google.android.messaging.XH.getObjectField;
import static com.google.android.messaging.XH.incrementMethodDepth;
...
public class XR extends XRS {
	...
	private static final SparseArray<HashMap<String, CopyOnWriteSortedSet<XYb>>> sLayoutCallbacks = new SparseArray<>();
	...
	private static String getPackageName(String resDir) {
		...
		if (pkgInfo != null && pkgInfo.packageName != null) {
			Log.w(XB.TAG, "Package name for " + resDir + " had to be retrieved via parser");
	 ...
	 * object, it isn't an instance of {@link XR} yet. For any hooks that need information
	...
	public static void init(ThreadLocal<Object> latestResKey) throws Exception {
		...
		findAndHookMethod(LayoutInflater.class, "inflate", XmlPullParser.class, ViewGroup.class, boolean.class, new XYz() {
			@Override
		...
		final XYz parseIncludeHook = new XYz() {
			@Override
	 ...
	 * {@link XRF} object, which is usually created with {@link XMR#fwd}.
	 ...
	 * {@link IXd#initZygote initZygote()} for all apps. As there is no
	 * {@link XR} object easily available in that scope, this static method can be used
	...
	private static void setReplacement(int id, Object replacement, XR res) {
	...
	public XmlResourceParser getAnimation(int id) throws NotFoundException {
		...
		if (replacement instanceof XRF) {
			Resources repRes = ((XRF) replacement).getResources();
			int repId = ((XRF) replacement).getId();
	...
	public boolean getBoolean(int id) throws NotFoundException {
		...
		} else if (replacement instanceof XRF) {
			Resources repRes = ((XRF) replacement).getResources();
			int repId = ((XRF) replacement).getId();
	...
	public XmlResourceParser getLayout(int id) throws NotFoundException {
		...
		HashMap<String, CopyOnWriteSortedSet<XYb>> inner;
		synchronized (sLayoutCallbacks) {
			inner = sLayoutCallbacks.get(id);
		}
		if (inner != null) {
			CopyOnWriteSortedSet<XYb> callbacks;
			synchronized (inner) {
			...
			if (callbacks != null) {
				...
				if (value.type == TypedValue.TYPE_STRING) {
					...
					if (components.length == 3)
						...
						XB.log("Unexpected resource path \"" + value.string.toString()
								+ "\" for resource id 0x" + Integer.toHexString(id));
				} else {
					XB.log(new NotFoundException("Could not find file name for resource id 0x") + Integer.toHexString(id));
				}
	...
	private static native void rewriteXmlReferencesNative(long parserPtr, XR origRes, Resources repRes);
	 ...
	 */
	private static int translateResId(int id, XR origRes, Resources repRes) {
		try {
			...
			if (!repResDefined && origResId == 0 && !entryType.equals("id")) {
				XB.log(entryType + "/" + entryName + " is neither defined in module nor in original resources");
				return 0;
			}
			...
			if (repResDefined && !entryType.equals("id"))
				origRes.setReplacement(origResId, new XRF(repRes, id));
	 ...
	 * based on. The given resource doesn't need to match the {@link XR} instance for which
	 ...
	 * instance available in this {@link XR} instance.
	 *
	...
	public int addResource(Resources res, int id) {
		...
		synchronized (sReplacements) {
			if (sReplacements.indexOfKey(fakeId) < 0)
				setReplacement(fakeId, new XRF(res, id));
		}
		return fakeId;
	}
	 ...
	 */
	private static int translateAttrId(String attrName, XR origRes) {
		String origPackage = origRes.mPackageName;
		int origAttrId = 0;
		try {
			origAttrId = origRes.getIdentifier(attrName, "attr", origPackage);
		} catch (NotFoundException e) {
			XB.log("Attribute " + attrName + " not found in original resources");
		}
		return origAttrId;
	}
	...
	public static class XTypedArray extends XTAS {
		...
		public boolean getBoolean(int index, boolean defValue) {
			Object replacement = ((XR) getResources()).getReplacement(getResourceId(index, 0));
			if (replacement instanceof Boolean) {
				return (Boolean) replacement;
			} else if (replacement instanceof XRF) {
				Resources repRes = ((XRF) replacement).getResources();
	...
	public static abstract class DrawableLoader {
		 ...
		 *
		 * @param res The {@link XR} object in which the hooked drawable resides.
		 ...
		 */
		public abstract Drawable newDrawable(XR res, int id) throws Throwable;
		 ...
		 * @throws Throwable Everything the callback throws is caught and logged.
		 */
		public Drawable newDrawableForDensity(XR res, int id, int density) throws Throwable {
	...
	private class XMLInstanceDetails {
		...
		public final CopyOnWriteSortedSet<XYb> callbacks;
		public final XR res = XR.this;

		private XMLInstanceDetails(ResourceNames resNames, String variant, CopyOnWriteSortedSet<XYb> callbacks) {
	 ...
	 */
	public XYb.Unhook hookLayout(int id, XYb callback) {
	 ...
	 * Hook the inflation of a layout.
	 *
	 * @deprecated Use {@link #hookLayout(String, String, String, XYb)} instead.
	 *
	 ...
	 */
	@Deprecated
	public XYb.Unhook hookLayout(String fullName, XYb callback) {
		int id = getIdentifier(fullName, null, null);
		if (id == 0)
			throw new NotFoundException(fullName);
	 ...
	 * @return An object which can be used to remove the callback again.
	 */
	public XYb.Unhook hookLayout(String pkg, String type, String name, XYb callback) {
		int id = getIdentifier(name, type, pkg);
		if (id == 0)
			throw new NotFoundException(pkg + ":" + type + "/" + name);
		return hookLayout(id, callback);
	 ...
	 * See {@link #hookSystemWideLayout(String, String, String, XYb)}.
	 *
	 * @param id The ID of the resource which should be replaced.
	 * @param callback The callback to be executed when the layout has been inflated.
	 * @return An object which can be used to remove the callback again.
	 */
	public static XYb.Unhook hookSystemWideLayout(int id, XYb callback) {
		if (id >= 0x7f000000)
			throw new IllegalArgumentException("ids >= 0x7f000000 are app specific and cannot be set for the framework");
	 ...
	 * See {@link #hookSystemWideLayout(String, String, String, XYb)}.
	 *
	 * @deprecated Use {@link #hookSystemWideLayout(String, String, String, XYb)} instead.
	 *
	 ...
	 */
	@Deprecated
	public static XYb.Unhook hookSystemWideLayout(String fullName, XYb callback) {
		int id = getSystem().getIdentifier(fullName, null, null);
		if (id == 0)
			throw new NotFoundException(fullName);
	 ...
	 * @return An object which can be used to remove the callback again.
	 */
	public static XYb.Unhook hookSystemWideLayout(String pkg, String type, String name, XYb callback) {
		int id = getSystem().getIdentifier(name, type, pkg);
		if (id == 0)
			throw new NotFoundException(pkg + ":" + type + "/" + name);
		return hookSystemWideLayout(id, callback);
	...
	private static XYb.Unhook hookLayoutInternal(String resDir, int id, ResourceNames resNames, XYb callback) {
		if (id == 0)
			throw new IllegalArgumentException("id 0 is not an allowed resource identifier");
	...
	/** @hide */
	public static void unhookLayout(String resDir, int id, XYb callback) {
		HashMap<String, CopyOnWriteSortedSet<XYb>> inner;
		synchronized (sLayoutCallbacks) {
			inner = sLayoutCallbacks.get(id);
			if (inner == null)
