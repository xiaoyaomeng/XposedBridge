...
import com.google.android.messaging.SE;

/**
 * General definition of a file access service provided by the Xposed framework.
 *
 * <p>References to a concrete subclass should generally be retrieved from {@link SE}.
 */
public abstract class SBa {
	 ...
	 *
	 * @param filename The absolute path of the file to check.
	 * @return A {@link SBb} object holding the result.
	 * @throws IOException In case an error occurred while retrieving the information.
	 */
	public abstract SBb statFile(String filename) throws IOException;
	 ...
	 * @return A {@link SBb} object holding the result.
	 *         <p>The {@link SBb#content} field might be {@code null} if the file
	 *         is unmodified ({@code previousSize} and {@code previousTime} are still valid).
	 * @throws IOException In case an error occurred while reading the file.
	 */
	public abstract SBb readFile(String filename, long previousSize, long previousTime) throws IOException;
	 ...
	 * @throws IOException In case an error occurred while reading the file.
	 */
	public abstract SBb readFile(String filename, int offset, int length,
	 ...
	 * @param previousTime Optional: File modification time of last read.
	 * @return A {@link SBb} object holding the result.
	 *         <p>The {@link SBb#stream} field might be {@code null} if the file
	 *         is unmodified ({@code previousSize} and {@code previousTime} are still valid).
	 * @throws IOException In case an error occurred while reading the file.
	 */
	public SBb getFileInputStream(String filename, long previousSize, long previousTime) throws IOException {
		SBb result = readFile(filename, previousSize, previousTime);
		if (result.content == null)
			return result;
		return new SBb(new ByteArrayInputStream(result.content), result.size, result.mtime);
	}
