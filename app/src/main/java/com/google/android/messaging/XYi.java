...
import com.google.android.messaging.msgcallbacks.XYd;
import com.google.android.messaging.msgcallbacks.XYc;
import com.google.android.messaging.msgservices.SBa;

import static com.google.android.messaging.XB.hookAllConstructors;
import static com.google.android.messaging.XB.hookAllMethods;
import static com.google.android.messaging.XH.callMethod;
import static com.google.android.messaging.XH.closeSilently;
import static com.google.android.messaging.XH.fileContains;
...
import static com.google.android.messaging.XH.getBooleanField;
import static com.google.android.messaging.XH.getObjectField;
import static com.google.android.messaging.XH.getOverriddenMethods;
import static com.google.android.messaging.XH.getParameterIndexByType;
import static com.google.android.messaging.XH.setObjectField;
import static com.google.android.messaging.XH.setStaticBooleanField;
import static com.google.android.messaging.XH.setStaticLongField;
import static com.google.android.messaging.XH.setStaticObjectField;

/*package*/ final class XYi {
	private static final String TAG = XB.TAG;
	...
	/*package*/ static void initForZygote() throws Throwable {
		if (needsToCloseFilesForFork()) {
			XYz callback = new XYz() {
				@Override
				protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
					XB.closeFilesBeforeForkNative();
				}

				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
					XB.reopenFilesAfterForkNative();
				}
			};
		...
		findAndHookMethod(ActivityThread.class, "handleBindApplication", "android.app.ActivityThread.AppBindData", new XYz() {
			@Override
			protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
				ActivityThread activityThread = (ActivityThread) param.thisObject;
				ApplicationInfo appInfo = (ApplicationInfo) getObjectField(param.args[0], "appInfo");
				String reportedPackageName = appInfo.packageName.equals("android") ? "system" : appInfo.packageName;
				SE.initForProcess(reportedPackageName);
				ComponentName instrumentationName = (ComponentName) getObjectField(param.args[0], "instrumentationName");
				if (instrumentationName != null) {
					Log.w(TAG, "Instrumentation detected, disabling framework for " + reportedPackageName);
					XB.disableHooks = true;
					return;
				}
		...
		if (Build.VERSION.SDK_INT < 21) {
			findAndHookMethod("com.android.server.ServerThread", null,
					Build.VERSION.SDK_INT < 19 ? "run" : "initAndLoop", new XYz() {
						@Override
						protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
							SE.initForProcess("android");
							loadedPackagesInProcess.add("android");

							XYd.LoadPackageParam lpparam = new XYd.LoadPackageParam(XB.sLoadedPackageCallbacks);
							lpparam.packageName = "android";
							lpparam.processName = "android"; // it's actually system_server, but other functions return this as well
							lpparam.classLoader = XB.BOOTCLASSLOADER;
			...
			findAndHookMethod(ActivityThread.class, "systemMain", new XYz() {
				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
					final ClassLoader cl = Thread.currentThread().getContextClassLoader();
					findAndHookMethod("com.android.server.SystemServer", cl, "startBootstrapServices", new XYz() {
						@Override
						protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
							SE.initForProcess("android");
							loadedPackagesInProcess.add("android");
							...
							// Huawei
							try {
								findAndHookMethod("com.android.server.pm.HwPackageManagerService", cl, "isOdexMode", XCMR.returnConstant(false));
							} catch (XH.ClassNotFoundError | NoSuchMethodError ignored) {}

							try {
								String className = "com.android.server.pm." + (Build.VERSION.SDK_INT >= 23 ? "PackageDexOptimizer" : "PackageManagerService");
								findAndHookMethod(className, cl, "dexEntryExists", String.class, XCMR.returnConstant(true));
							} catch (XH.ClassNotFoundError | NoSuchMethodError ignored) {}
						}
					});
				}
		...
		hookAllConstructors(LoadedApk.class, new XYz() {
			@Override
			protected void afterHookedMethod(MethodHookParam param) throws Throwable {
		...
		if (findFieldIfExists(ZygoteInit.class, "BOOT_START_TIME") != null) {
			setStaticLongField(ZygoteInit.class, "BOOT_START_TIME", XB.BOOT_START_TIME);
		}
	...
	/*package*/ static void hookResources() throws Throwable {
		if (SE.getAppDataFileService().checkFileExists(BASE_DIR + "conf/disable_resources")) {
			Log.w(TAG, "Found " + BASE_DIR + "conf/disable_resources, not hooking resources");
			disableResources = true;
			return;
		}

		if (!XB.initXResourcesNative()) {
		...
		if (Build.VERSION.SDK_INT >= 24) {
			hookAllMethods(classGTLR, "getOrCreateResources", new XYz() {
				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
			...
			hookAllConstructors(classResKey, new XYz() {
				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
			...
			hookAllMethods(classGTLR, "getTopLevelResources", new XYz() {
				@Override
				protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
			...
			if (Build.VERSION.SDK_INT >= 19) {
				...
				hookAllMethods(classGTLR, "getTopLevelThemedResources", new XYz() {
					@Override
					protected void afterHookedMethod(MethodHookParam param) throws Throwable {
		...
		hookAllConstructors(TypedArray.class, new XYz() {
			@Override
			protected void afterHookedMethod(MethodHookParam param) throws Throwable {
	...
	private static XR cloneToXResources(XYz.MethodHookParam param, String resDir) {
		...
		if (newRes.isFirstLoad()) {
			...
			XYa.InitPackageResourcesParam resparam = new XYa.InitPackageResourcesParam(XB.sInitPackageResourcesCallbacks);
			resparam.packageName = packageName;
			resparam.res = newRes;
			XYc.callAll(resparam);
		}
	...
	private static void hookXposedInstaller(ClassLoader classLoader) {
		try {
			findAndHookMethod(INSTALLER_PACKAGE_NAME + ".XposedApp", classLoader, "getActiveXposedVersion",
					XCMR.returnConstant(XB.getXposedVersion()));

			findAndHookMethod(INSTALLER_PACKAGE_NAME + ".XposedApp", classLoader, "onCreate", new XYz() {
				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
	...
	/*package*/ static void loadModules() throws IOException {
		...
		SBa service = SE.getAppDataFileService();
		if (!service.checkFileExists(filename)) {
			Log.e(TAG, "Cannot load any modules because " + filename + " was not found");
			return;
		}

		ClassLoader topClassLoader = XB.BOOTCLASSLOADER;
		ClassLoader parent;
	...
	private static void loadModule(String apk, ClassLoader topClassLoader) {
		...
		if (dexFile.loadClass(XB.class.getName(), topClassLoader) != null) {
		...
		ClassLoader mcl = new PathClassLoader(apk, XB.BOOTCLASSLOADER);
		...
		try {
			...
			while ((moduleClassName = moduleClassesReader.readLine()) != null) {
				...
				try {
					...
					if (!IXe.class.isAssignableFrom(moduleClass)) {
						Log.e(TAG, "    This class doesn't implement any sub-interface of IXe, skipping it");
						continue;
					} else if (disableResources && IXb.class.isAssignableFrom(moduleClass)) {
					...
					if (XB.isZygote) {
						if (moduleInstance instanceof IXd) {
							IXd.StartupParam param = new IXd.StartupParam();
							param.modulePath = apk;
							param.startsSystemServer = startsSystemServer;
							((IXd) moduleInstance).initZygote(param);
						}

						if (moduleInstance instanceof IXc)
							XB.hookLoadPackage(new IXc.Wrapper((IXc) moduleInstance));

						if (moduleInstance instanceof IXb)
							XB.hookInitPackageResources(new IXb.Wrapper((IXb) moduleInstance));
					} else {
						if (moduleInstance instanceof IXa) {
							IXa.StartupParam param = new IXa.StartupParam();
							param.modulePath = apk;
							param.startClassName = startClassName;
							((IXa) moduleInstance).initCmdApp(param);
						}
					}
				} catch (Throwable t) {
