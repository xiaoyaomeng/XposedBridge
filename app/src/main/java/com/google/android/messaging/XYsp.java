...
import com.google.android.messaging.msgservices.SBb;
 ...
 */
public final class XYsp implements SharedPreferences {
	private static final String TAG = "XYsp";
	 ...
	 */
	public XYsp(String packageName) {
	 ...
	 */
	public XYsp(String packageName, String prefFileName) {
	...
	public boolean makeWorldReadable() {
		if (!SE.getAppDataFileService().hasDirectFileAccess())
	...
	private void startLoadFromDisk() {
		...
		new Thread("XYsp-load") {
			@Override
			public void run() {
				synchronized (XYsp.this) {
					loadFromDiskLocked();
				}
			}
		}.start();
	...
	private void loadFromDiskLocked() {
		...
		SBb result = null;
		try {
			result = SE.getAppDataFileService().getFileInputStream(mFilename, mFileSize, mLastModified);
	...
	public synchronized boolean hasFileChanged() {
		try {
			SBb result = SE.getAppDataFileService().statFile(mFilename);
